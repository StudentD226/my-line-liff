...
export default function ResidentNewsFeed() {
  ...
  // ดึงข้อมูลจาก API ตัวเดิมที่เราสร้างไว้
  const fetchNews = async () => {
    ...
    try {
      ...
      if (json.success) {
        // กรองเอาเฉพาะข่าวที่ "เผยแพร่แล้ว (PUBLISHED)" เท่านั้นมาโชว์ให้ลูกบ้านเห็น
        const publishedNews = json.data.filter((n: NewsItem) => n.status === 'PUBLISHED');
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-10">
      ...
      {/* Popup อ่านรายละเอียดเต็ม (Full Screen Modal) */}
      {selectedNews && (
