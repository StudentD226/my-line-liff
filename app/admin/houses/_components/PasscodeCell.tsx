    ...
    <div className="flex items-center justify-between bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl w-max font-mono text-sm font-bold text-slate-700 shadow-sm">
      ...
      <div className="flex items-center gap-1 border-l border-slate-200 pl-2 ml-2">
        ...
        <button
          type="button"
          onClick={handleCopy}
          className={`p-1.5 rounded-lg transition-colors border shadow-sm ${copied ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-white text-slate-400 hover:text-[#1A534B] hover:bg-slate-100 border-slate-200"}`}
          title="คัดลอกรหัสลับ"
        >
        ...
        <button
          ...
          onClick={handleReset}
          disabled={isPending}
          className={`p-1.5 rounded-lg border border-slate-200 shadow-sm transition-colors bg-white text-slate-400 hover:text-orange-600 hover:bg-orange-50 ${isPending ? "opacity-50" : ""}`}
