...
import { Settings, Trash2, Plus, Edit, RefreshCw, Send, AlertCircle, Clock, Search, Filter, Receipt, X, ChevronLeft, ChevronRight } from 'lucide-react';
          ...
          {selectedInvoices.length > 0 && (
            <div className="bg-gradient-to-r from-teal-50 to-emerald-50 px-6 py-4 border-b border-teal-100 flex flex-col md:flex-row justify-between items-center gap-4 animate-in fade-in z-10 relative shadow-sm">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="w-px h-6 bg-teal-200 mx-2 hidden sm:block"></div>
                <button onClick={() => handleDelete()} className="flex items-center px-4 py-2 bg-white hover:bg-red-50 text-red-600 border border-red-200 text-xs font-bold rounded-xl transition shadow-sm hover:shadow active:scale-[0.98]">
            ...
            <table className="w-full text-left border-collapse min-w-[900px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100 text-gray-500">
                  ...
                  {/* 🌟 1. สลับคอลลัมน์หัวตาราง ย้าย "จัดการบิล" มาไว้ก่อน "แจ้งเตือนผ่าน LINE" */}
                  ...
                  filteredInvoices.map((inv) => {
                    ...
                    return (
                      <tr key={inv.id} className={`transition-colors hover:bg-gray-50/50 ${isSelected ? 'bg-teal-50/30' : ''}`}>
                        ...
                        {/* 🌟 2. สลับบล็อกปุ่มเนื้อหา ย้ายปุ่มกลุ่ม แก้ไข/รีเซ็ต/ลบ ขึ้นมาไว้ก่อนแถบปุ่มส่ง LINE */}
