...
<<<<<<< HEAD
        ...
        await prisma.invoice.update({
          where: { id: invoiceId },
          data: { penaltyAmount: displayPenalty, totalAmount: truncateDecimals(invoice.baseAmount + displayPenalty), status: 'OVERDUE' }
        });
      }
    } else if (type === 'SEND') {
...
<<<<<<< HEAD
    ...
    const flexBubble = createInvoiceFlexMessage({
      ...
      dueDateText,
      invoiceNo: invoice.id // 🌟 ส่ง ID ไปโชว์ใน Payment ID ตรง Footer
    });
