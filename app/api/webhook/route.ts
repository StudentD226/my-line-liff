...
export async function POST(request: Request) {
  try {
    ...
    // คำนวณค่าปรับเฉพาะบิลนี้ (ถ้าถูกสั่งให้ทวงแบบ OVERDUE)
    if (type === 'OVERDUE') {
    ...
    const allUnpaidInvoices = await prisma.invoice.findMany({
      where: {
        ...
        billingYear: { not: 9999 } // ตัดบิลสลิปที่รอเช็กออกไป
      },
    ...
    allUnpaidInvoices.forEach(inv => {
      // 🌟 พระเอกอยู่ตรงนี้ครับ! กรองเอาเฉพาะบิลนี้ และบิลที่เก่ากว่าบิลนี้เท่านั้น (ไม่เอาบิลอนาคตมารวม)
      ...
      if (isTargetInvoice || isOlderInvoice) {
        ...
        // หักลบยอดที่เคยทยอยจ่ายมาแล้ว (Partial Logic)
        if (paid > 0) {
          if (paid >= penalty) {
        ...
        // ถ้ายอดยังเหลือให้จ่าย ค่อยนำมาโชว์
        if (base > 0 || penalty > 0) {
          grandTotalBase += base;
